...
// Get system-wide basic redundancy information for HA Appliances
func (e *Semp) GetRedundancyHWSemp1(ch chan<- PrometheusMetric) (ok float64, err error) {
	var f float64
	...
	body, err := e.postHTTP(e.brokerURI+"/SEMP", "application/xml", command, "RedundancyHWSemp1", 1)
	if err != nil {
		_ = level.Error(e.logger).Log("msg", "Can't scrape RedundancyHWSemp1", "err", err, "broker", e.brokerURI)
		return 0, err
	}
	defer body.Close()
	...
	if err != nil {
		_ = level.Error(e.logger).Log("msg", "Can't decode Xml RedundancyHWSemp1", "err", err, "broker", e.brokerURI)
		return 0, err
	}
